  ...
  useEffect(()=>
    { 
      props.controller.itemController.value[props.controller.index] = (args,params)=>
      {
          ...
          else if(args == 'BOMB-DIRECT-EXPLOSION')
          {
            let bombExplosion = ()=>
              {
                if(itemGroupRef.current.children[2].scale.x >= 3.5)
                {
                  ...
                  _gameAppContext.checkBlastArea(params)
                  return true;
                }
                else
                {
      ...
      >
          ...
          {props.skin == "box_1" && <group  ref={itemRef}>
                                            ...
                                            {props.customModel!='none'?
                                              ...
                                              :
                                              <>
                                                <mesh scale={1} geometry={nodes.box2.geometry} material={new THREE.MeshMatcapMaterial({color:'blue'})} position={[0,0.8,0]} /> 
                                              </>
                                            } 
                                    </group>
          }
...
export function Barier_Model(props)
{
  ...
  return(     <>
                     {props.customModel == 'none'?
                      <mesh ref={modelRef} name="nbarrier" rotation={[0,(props.orientation == 'HORIZONTAL'?0:Math.PI*0.5),0]} geometry={nodes.barrier.geometry} material={new THREE.MeshMatcapMaterial({color:'gray'})} position={[props.x,0,props.z]} >
                          <mesh ref={planeRef} name="nbarrierplane" geometry={nodes.barrierShad.geometry} material={mat2} position={[0, 1.048, 0]} />
                      </mesh>
                      :
                      <group ref={modelRef} position={[props.x,0,props.z]} rotation={[0,(props.orientation == 'HORIZONTAL'?0:Math.PI*0.5),0]}>
                        {props.customModel}
                      </group>
                     }
...
export function TestModel(props)
{
  ...
  return(
          <>
              ...
              <mesh geometry={nodes.mousemob.geometry} material={new THREE.MeshMatcapMaterial({color:'gray'})} position={[props.x,0,props.z]}>
                <mesh geometry={nodes.mousemobEye.geometry} material={new THREE.MeshMatcapMaterial({color:'red'})} />
              </mesh>
          </>
  )
}
...
export function BallPortalModel(props)
{
  ...
  let partalShaders = new THREE.ShaderMaterial({transparent:true,side:THREE.DoubleSide,vertexShader:portal2vertex,fragmentShader:portal2frags,
                                                uniforms:{utime:{value:0},portalCol:{value:new THREE.Vector3(1,0,0)}}})
  let _utime = 0;
  useFrame(()=>
  {
    _utime += 0.01;
  ...
  useEffect(()=>
    { 
      props.controller.itemController.value[props.controller.index] = (args)=>
      {
          if(args == 'BLUE')
          {
            partalShaders.uniforms.portalCol.value = new THREE.Vector3(1,1,1);
          }
      }
    },[])
  return(
